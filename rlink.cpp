...
extern bool balance; //// switch for controling whether balance the splice or not, true for balance, false for not balance
...
void eliminate_transfrags_under_thr(int gno,GIntHash<int>& gpos,GPVec<CTransfrag>& transfrag, CTreePat *tr2no,float threshold, bool newstep = false) {
	...
	int folder = int(transfrag.Count()/max_trf_number);
	if(newstep){
		if(folder <= 1)
			threshold ++;
		else
			threshold = threshold * sqrt(folder);
	}
	int precount = 0;
	float prethr = threshold;

	while (transfrag.Count() > max_trf_number && !(newstep && precount == transfrag.Count() && prethr != threshold)) {
		precount = transfrag.Count();
		for(int t=transfrag.Count()-1;t>=0;t--)
		...
		if(!newstep)
			...
			folder = int(transfrag.Count() / max_trf_number);
			if (folder <= 1)
				threshold++;
			else
				threshold = threshold*sqrt(folder);
...
CTransfrag *find_guide_pat(GffObj *guide,GPVec<CGraphnode>& no2gnode,int gno,int edgeno,GIntHash<int> &gpos, int& lastgpos) {
	...
	while(i<gno-1) {
		if(no2gnode[i]->overlap(guide->exons[0])) { // first exon of guide overlaps node i
			...
			while(j<gno-1 && no2gnode[j]->overlap(guide->exons[0]) && no2gnode[j]->start==no2gnode[j-1]->end+1
					&& no2gnode[j-1]->child.Count() && no2gnode[j-1]->child[0]==j) j++;	//// skip a continuous region
	...
	if(i<gno-1) { // found start node
		...
		while(j<n) {
			if(guide->exons[j]->end<inode->end) { ////  all the exon's end must match except the last one's
				...
				j++;	//// current exon's end has been matched, go to next guide's exon
				int k=0;
				int nc=inode->child.Count();
				while(k<nc && no2gnode[inode->child[k]]->start != guide->exons[j]->start) k++;	//// find next node whose start matches the start of exon j
				if(k==nc){	//// if not found, return 
					if(!trguide)
						break;
					return(trguide);
				}
				...
				i=inode->child[k];	//// go to next node
				nodes.Add(i);
				inode=no2gnode[i];
			}
			else {
	...
	{//// newly added
		int i = 0; //// index of exons in guide
		int j = 1; //// index of nodes in graph
		GVec<int> nodes;
		while(i < guide->exons.Count() && j < gno - 1){//// search the nodes that covered by current guide
			if(no2gnode[j]->start >= guide->exons[i]->end){
				i++;
				continue;
			}
			if(no2gnode[j]->end <= guide->exons[i]->start){
				j++;
				continue;
			}
			//// then there is overlap between current exon and node
			if(no2gnode[j]->cov/no2gnode[j]->len() < readthr && no2gnode[j]->len() >= bundledist)
				return trguide; //// if the node is not supported by read coverage, drop the guide
			nodes.Add(j);
			j++;
			if(no2gnode[j]->end >= guide->exons[i]->end)
				i++;
		}
		if(nodes.Count()==0 ||(i < guide->exons.Count() && !(j== gno - 1 && no2gnode[j-2]->end < guide->exons[i]->end)))//// there are exons that not supported by the sequencing data, so drop the guide
			return trguide;
		for(int i = 0; i < nodes.Count() - 1; i++){
			int* pos = gpos[edge(nodes[i], nodes[i+1], gno)];
			if(!pos){
				edgeno += 1;
			}
		}
		GBitVec newguidepath(gno + edgeno);
		newguidepath[nodes[0]] = 1;
		for(int i = 1; i < nodes.Count(); i++){
			newguidepath[nodes[i]] = 1;
			int* pos = gpos[edge(nodes[i-1], nodes[i], gno)];
			if(!pos){
				int key = edge(nodes[i-1], nodes[i], gno);
				//fprintf(stderr, "key = %d, %d - %d\n", key, nodes[i-1], nodes[i]);
				gpos.Add(key, lastgpos);
				lastgpos++;
				pos = gpos[key];
				no2gnode[nodes[i-1]]->child.Add(nodes[i]);
				no2gnode[nodes[i-1]]->child.Sort();
				no2gnode[nodes[i]]->parent.Add(nodes[i-1]);
				no2gnode[nodes[i]]->parent.Sort();
			}
			newguidepath[*pos] = 1;
		}
		trguide = new CTransfrag(nodes, newguidepath);
		return(trguide);
	}
...
void process_refguides(int gno,int edgeno,GIntHash<int>& gpos,int& lastgpos,GPVec<CGraphnode>& no2gnode,
	...
	for(int g=0;g<guides.Count();g++) {
		...
		if((guides[g]->strand==strand || guides[g]->strand=='.') && ((RC_TData*)(guides[g]->uptr))->in_bundle>=2 && (guides[g]->overlap(no2gnode[1]->start,no2gnode[gno-2]->end))) {
			CTransfrag *trguide=find_guide_pat(guides[g],no2gnode,gno,edgeno,gpos, lastgpos);
	...
	*/
	...
	while(g<guidetrf.Count()) {
		...
		if(nodei) { // if source it isn't already in guide's pattern -> it shouldn't be
			...
			//// newly added
			if((uint)(*pos) >= guidetrf[g].trf->pattern.size()){
				totalnum = *pos + 1;
				edgeno = totalnum - gno;
				GBitVec maskbit(totalnum);
				guidetrf[g].trf->pattern |= maskbit;
			}
...
void print_flux(int gno, GIntHash<int>& gpos, GVec<float>& influx, GVec<float>& outflux, GVec<float>& thrflux, GPVec<CGraphnode>& no2gnode)
{
	for(int i = 0; i < gno; i++){
		CGraphnode* inode = no2gnode[i];
		fprintf(stderr, "node %d(%d-%d):\t ", i, inode->start, inode->end);
		int cn = inode->child.Count();
		for(int j = 0; j < cn; j++)
			fprintf(stderr, "%d \t", inode->child[j]);
		fprintf(stderr,"\n");
	}

	float fluxmat[gno + 1][gno + 1];
	for(int i = 0; i <= gno; i++)
		for(int j = 0; j <= gno; j++){
			fluxmat[i][j] = 0;
			if(i == j && i < gno)
				fluxmat[i][j] = thrflux[i];
		}
	for(int i = 0; i < gno; i++){
		CGraphnode* node1 = no2gnode[i];
		for(int j = 0; j < gno; j++){
		 	CGraphnode* node2 = no2gnode[j];
			int* pos = gpos[edge(i, j, gno)];
			if(pos && *pos >= gno && node1->childpat[j] && node2->parentpat[i]){
				fluxmat[i][gno] += thrflux[*pos];
				fluxmat[gno][j] += thrflux[*pos];
				fluxmat[i][j] = thrflux[*pos];
			}
		}
	}
	...
	for(int i = 0; i <= gno; i++){
		if( i < gno ){
			fprintf(stderr, "nod_%d\t", i);
		}
		else{
			fprintf(stderr,"calout\t outflux\t influx\n");
		}
	}
	for(int i = 0; i <= gno; i++){
		for(int j = 0; j <= gno; j++){
			if( j == 0 && i < gno)
				...
				fprintf(stderr, "calin\t");
			if(fluxmat[i][j] > 0.0)
				fprintf(stderr, "%.4f\t", fluxmat[i][j]);
			else
				fprintf(stderr, "\t");
			if(j == gno && i < gno){
				if(outflux[i] > 0)
					fprintf(stderr, "%.4f\t", outflux[i]); //// print influx for each node
				else
					fprintf(stderr, "\t");
				if(influx[i] > 0)
					fprintf(stderr, "%.4f\n", influx[i]);
	...
	for(int i = 0; i < gno; i++)
		if(influx[i] > 0.0)
			fprintf(stderr, "%.4f\t", influx[i]); //// print original in flux for each node
		else
			fprintf(stderr, "\t");
	fprintf(stderr, "\n");
}

bool is_good_solution(GVec<float>& preinflux, GVec<float>& preoutflux, GVec<float>& prethrflux, GVec<float>& influx, GVec<float>& outflux, GVec<float>& thrflux)
{
	int nodeno = influx.Count();
	int allno = thrflux.Count();
	float gapthreshold = epsilon;
	for(int i = 0; i < allno; i++){
		if(i < nodeno){
			if(abs(preinflux[i] - influx[i]) > gapthreshold || abs(preoutflux[i] - outflux[i]) > gapthreshold || abs(prethrflux[i] - thrflux[i]) > gapthreshold)
				return false;
		}
		else
			if( abs(prethrflux[i] - thrflux[i]) > gapthreshold)
				return false;
	}
	return true;
}

void Cplex_Lp_Solver(int gno, int edgeno, GIntHash<int>& gpos, GVec<float>& influx, GVec<float>& outflux, GVec<float>& thrflux, GPVec<CGraphnode>& no2gnode)
{//// use CPLEX library to solve linear programming problem
	IloEnv	cpl_env;
	try{
		IloModel cpl_model(cpl_env);
		IloNumVarArray eps_array(cpl_env, gno + edgeno, 0.0, IloInfinity);
				////	envoriment, size, 	lower bound, upper bound
		IloNumVarArray flux_array(cpl_env, gno + edgeno, 0.0, IloInfinity);
		cpl_model.add(IloMinimize(cpl_env, IloSum(eps_array)));

		for(int i = 0; i < gno + edgeno; i++){//// for each node & edge, the change is limited
				cpl_model.add(flux_array[i] - thrflux[i] <= eps_array[i]);
				cpl_model.add(flux_array[i] - thrflux[i] >= -eps_array[i]);
		}
		for(int i = 0; i < gno; i++){
			IloNumExprArray in_array(cpl_env);
			IloNumExprArray out_array(cpl_env);
			for(int child_idx = 0; child_idx < no2gnode[i]->child.Count(); child_idx++){
				int child = no2gnode[i]->child[child_idx];
				int* pos = gpos[edge(i, child, gno)];
				out_array.add(flux_array[*pos]);
			}
			for(int parent_idx = 0; parent_idx < no2gnode[i]->parent.Count(); parent_idx++){
				int parent = no2gnode[i]->parent[parent_idx];
				int* pos = gpos[edge(parent, i, gno)];
				in_array.add(flux_array[*pos]);
			}

			//// for each node, keep the flows balance
			
			if(i == 0)
				cpl_model.add(flux_array[i] == IloSum(out_array));
			else if(i == gno - 1)
				cpl_model.add(flux_array[i] == IloSum(in_array));
			else{
				cpl_model.add(flux_array[i] == IloSum(out_array));
				cpl_model.add(flux_array[i] == IloSum(in_array));
			}
		}
		IloCplex cplex(cpl_model);
		cplex.setOut(cpl_env.getNullStream()); //// turn off display log message
		cplex.solve();
		IloNumArray values(cpl_env);
		cplex.getValues(values, flux_array);

		//// assign new value back
		for(int i = 0; i < gno + edgeno; i++){
			////fprintf(stderr, "%d-th: %f\n",i, values[i]);
			thrflux[i] = values[i];
			if(i < gno ){
				if(no2gnode[i]->child.Count()){
					outflux[i] = 0;
					for(int child_idx = 0; child_idx < no2gnode[i]->child.Count(); child_idx++){
						int child = no2gnode[i]->child[child_idx];
						int* pos = gpos[edge(i, child, gno)];
						outflux[i] += values[*pos];
						////fprintf(stderr, "%d(%d-%d): %f\n", *pos, i, child, values[*pos]);
					}
				}
				if(no2gnode[i]->parent.Count()){
					influx[i] = 0;
					for(int parent_idx = 0; parent_idx < no2gnode[i]->parent.Count(); parent_idx++){
						int parent = no2gnode[i]->parent[parent_idx];
						int* pos = gpos[edge(parent, i, gno)];
						influx[i] += values[*pos];
						////fprintf(stderr, "%d(%d-%d): %f\n", *pos, parent, i, values[*pos]);
					}
				}
			}
		}
...
void balance_graph(int gno, int edgeno, GIntHash<int>& gpos, GPVec<CTransfrag>& transfrag, GVec<float>& influx, GVec<float>& outflux, GVec<float>& thrflux, GPVec<CGraphnode>& no2gnode, CTreePat* tr2no)////, GVec<int>& toporder)
{//// balance the flow graph. gno: the number of nodes in graph, gpos: index of edges, influx: flows go in a node, outflux: flows go out of a node, thrflux: flows go through a node, no2gnode: index of nodes
////	topo_sort(gno, no2gnode, toporder);
	//int iters = 0;
	//float oriout = outflux[0];
	//float oriin = influx[gno-1];
	//int tmpc = 0;
	GVec<float> oriinflux = influx;
	GVec<float> orioutflux = outflux;
	GVec<float> orithrflux = thrflux;
	Cplex_Lp_Solver(gno, edgeno, gpos, influx, outflux, thrflux, no2gnode);
	int lasttidx = transfrag.Count();
	int addno = 0;
	int minusno = 0;
	for(int i = 0; i < gno - 1; i++){
		int cn = no2gnode[i]->child.Count();
		if(cn){
			for(int c = 0; c < cn; c++){
				int cidx = no2gnode[i]->child[c];
				int* pos = gpos[edge(i, cidx, gno)];
				if(pos && orithrflux[*pos] != thrflux[*pos]){
					float gapflux = abs(orithrflux[*pos] - thrflux[*pos]);
					if(orithrflux[*pos] < thrflux[*pos]){ //// need to make up the flow
						GVec<int> nodes;
						nodes.Add(i);
						nodes.Add(cidx);
						GBitVec trpat(gno + edgeno);
						...
						trpat[*pos] = 1;
						bool found = false;
						for(int ti = 0; ti < transfrag.Count(); ti++){
							if(trpat == transfrag[ti]->pattern){
								transfrag[ti]->abundance += gapflux;
								found = true;
								break;
							}
						}
						if(!found){
							CTransfrag* tr = new CTransfrag(nodes, trpat, gapflux);
							transfrag.Add(tr);
							no2gnode[i]->trf.Add(lasttidx);
							no2gnode[cidx]->trf.Add(lasttidx);
							lasttidx += 1;
							addno += 1;
						}
					}
					else{ //// need to decrease the flow
						int trfcount = no2gnode[c]->trf.Count();
						for(int t = trfcount - 1; t > -1 ; t--){ //// adapt the flux by minus
							if(gapflux && transfrag[t]->nodes.Count() > 1){ //// if there is still gapflux to adapt
								if(transfrag[t]->nodes[0] == i && transfrag[t]->nodes.Last() == cidx && transfrag[t]->nodes.Count() == 2){ //// found a transfrag start from current node and end at cidx
									if(transfrag[t]->abundance >= gapflux){
										transfrag[t]->abundance -= gapflux;
										gapflux = 0;
									}
									else{
										transfrag[t]->abundance = 0;
										gapflux -= transfrag[t]->abundance;
									}
								}else if(transfrag[t]->nodes.Last() > cidx){
									int trabund = 0;
									if(transfrag[t]->abundance >= gapflux){
										transfrag[t]->abundance -=gapflux;
										trabund = gapflux;
									...
									GVec<int> nodes;
									GBitVec trpat(gno + edgeno);
									for(int n = 0; n < transfrag[t]->nodes.Count(); n++){
										int nidx = transfrag[t]->nodes[n];
										if(nidx != i && nidx != cidx){
											nodes.Add(nidx);
											trpat[nidx] = 1;
										}
										if(n && n < transfrag[t]->nodes.Count() - 1 &&(nidx < i || preidx > cidx)){
											int* pos = gpos[edge(preidx, nidx, gno)];
											if(pos)
												trpat[*pos] = 1;
										}
										preidx = nidx;
									...
									if(!found){
										CTransfrag* tr = new CTransfrag(nodes, trpat, trabund);
										transfrag.Add(tr);
										for(int n = 0; n < nodes.Count(); n++){
											int nidx = nodes[n];
											no2gnode[nidx]->trf.Add(lasttidx);
										}
										lasttidx += 1;
										minusno += 1;
									}
	...
	}//// end adjust transfrags
	//// process transfrag
	eliminate_transfrags_under_thr(gno, gpos, transfrag, tr2no, trthr, true);
	transfrag.Sort(trCmp);
	GVec<int> incompletetrf; //remembers incomplete transfrags (the ones that don't have edges between two consecutive nodes

	// create compatibilities
	for(int i = 0; i < gno; i++){
		no2gnode[i]->trf.Clear();	
	}
	for (int t1 = 0; t1 < transfrag.Count(); t1++) { // transfrags are processed in increasing order -> important for the later considerations

		// update nodes
		int n1 = transfrag[t1]->nodes.Count();
		transfrag[t1]->path.Clear();
		transfrag[t1]->usepath = -1;
		...
		if (n1 > 1) { // add transfrag to nodes' in and out; if a transfrag only has one node then it is node added to a node; I might want to change this for the computation of fpkm's
			...
			if (incomplete)
				incompletetrf.Add(t1);
			else
				transfrag[t1]->real = true;
		}
	}
	CGraphnode* source = no2gnode[0];
	for (int i = 0; i < source->child.Count(); i++) {
		...
		if (no2gnode[source->child[i]]->parent.Count() == 1 && !no2gnode[source->child[i]]->parent[0]) { // source is the only parent of node
			for (int j = 0; j < no2gnode[source->child[i]]->trf.Count(); j++) {
				int t = no2gnode[source->child[i]]->trf[j];
				if (transfrag[t]->nodes.Last() == source->child[i])
					t0 = t;
				else
					abundance += transfrag[t]->abundance;
			}
			if (t0 > -1) { // found transfrag from source to node
				transfrag[t0]->abundance = abundance;
			}
...
int find_transcripts(int gno,int edgeno, GIntHash<int> &gpos,GPVec<CGraphnode>& no2gnode,GPVec<CTransfrag>& transfrag,GBitVec& compatible,
		int geneno,int strand,GVec<CGuide>& guidetrf,GList<CPrediction>& pred,GPVec<GffObj>& guides,GVec<int>& guidepred,BundleData* bdata, CTreePat* tr2no) {

	// process in and out coverages for each node
	int maxi=0; // node with maximum coverage
	GVec<float> nodecov; // node coverages
	GVec<float> influx(gno, 0.0);  //// in flux for each node
	GVec<float> outflux(gno, 0.0); //// out flux for each node
	GVec<float> thrflux(gno + edgeno, 0.0); ////flux go through each node and edge
	for(int t = 0; t < transfrag.Count(); t++){//// compute the flux on each edge
		int nnodes = transfrag[t]->nodes.Count();
		if(nnodes > 1){
			for(int n = 0; n < nnodes - 1; n++){
				int node1 = transfrag[t]->nodes[n];
				int node2 = transfrag[t]->nodes[n+1];
				/*
				if(node1 == 0 || node2 == gno - 1){
					fprintf(stderr, "trans: %d(%d - %d)\t %f\n", t, node1, node2, transfrag[t]->abundance)
					continue; //// skip the transfrags that link to source or sink
				}
				*/
				int* pos = gpos[edge(node1, node2, gno)];
				if(pos && transfrag[t]->pattern[*pos] && no2gnode[node1]->childpat[node2] && no2gnode[node2]->parentpat[node1]){
					outflux[node1] += transfrag[t]->abundance;
					influx[node2] += transfrag[t]->abundance;
					thrflux[*pos] += transfrag[t]->abundance;
					if(node1 == 0)
						thrflux[node1] += transfrag[t]->abundance;
					if(node2 == gno - 1)
						thrflux[node2] += transfrag[t]->abundance;
				}
			}
		}
	}
	...
	////fprintf(stderr, "-----before balancing\n");
	////print_flux(gno, gpos, influx, outflux, thrflux, no2gnode);
	if(balance)
		balance_graph(gno, edgeno, gpos, transfrag, influx, outflux, thrflux, no2gnode, tr2no);////, toporder);
	////fprintf(stderr, "-----After balancing\n");
	////print_flux(gno, gpos, influx, outflux, thrflux, no2gnode);
	/*
	for(int i = 0; i < transfrag.Count(); i++){
		fprintf(stderr, "transfrag-%d(%f), nodes: ", i, transfrag[i]->abundance);
		for(int j = 0; j < transfrag[i]->nodes.Count(); j++)
			fprintf(stderr, "%d-", transfrag[i]->nodes[j]);
		fprintf(stderr, "\n");
	}
	*/
...
*/
	...
	/*
	   {////DEBUG ONLY
	   for(int t = 0; t < transfrag.Count(); t++){
	   fprintf(stderr, "%d transfrag(%d nodes): %.4f\n", t, transfrag[t]->nodes.Count(), transfrag[t]->abundance);
	   }
	   }
	   */
	...
	if(nodecov[maxi]>=readthr) {
		...
		if(!eonly && nodecov[maxi]>=readthr*2) {
...
*/
bool Is_Bundle_Support_Guide(int g, int startnode, GPVec<CBundlenode>& bnode, GPVec<GffObj>& guides, GBitVec& supportState, GHash<bool>* boundary_strand)
{//// If there is no any bnode overlaps with any exon of guide, return false, else return true
//// test if a bundle support a guide(for each bundle's bnode overlapping with the guide, the bnode must contain the corresponding exon)
	//// if all the exons have supporting bnode, then the guide is supported
	CBundlenode* curr_bnode = bnode[startnode];
	CBundlenode* prev_bnode = NULL;
	for(int e = 0; e < guides[g]->exons.Count(); e++){
		GSeg *curr_exon = guides[g]->exons[e];
		//fprintf(stderr, "   current %d-th exon(%d, %d) \t bnode(%d, %d)\n", e, curr_exon->start, curr_exon->end, curr_bnode->start, curr_bnode->end);
		if(curr_exon->end < curr_bnode->start) //// dont have overlap region with current bnode
			continue;
		while(curr_bnode != NULL && !curr_exon->overlap(curr_bnode->start, curr_bnode->end)){ //// find the overlap bnode first
			if(curr_exon->end < curr_bnode->start && (prev_bnode == NULL || prev_bnode->end < curr_exon->start)) //// exon exists in the interval between two bnodes, skip this exon
				break;
			prev_bnode = curr_bnode;
			curr_bnode = curr_bnode->nextnode;
		}
		if(curr_bnode == NULL){ //// reach the end of current bundle
			if(supportState.any())
				return true;
			else
				return false;
		}
		if(curr_exon->end < curr_bnode->start && (prev_bnode == NULL || prev_bnode->end < curr_exon->start)){
			continue;
		}
		GStr e_start((int)curr_bnode->start);
		GStr e_end((int)curr_bnode->end);
		
		if((*boundary_strand)[e_start.chars()] && curr_exon->start < curr_bnode->start)
			//// current bnode is not extenable,
			return false;
		if((*boundary_strand)[e_end.chars()] && curr_exon->end > curr_bnode->end)
			return false;
		supportState[e] = 1; //// then the exon is supported by bnode
	} 
	if(supportState.any())
		return true;
	else
	...
	/*
	int lastexon = 0;
	CBundlenode* bundlenode = bnode[startnode];
	while(bundlenode != NULL){
		for(int exn = lastexon; exn < guides[g]->exons.Count(); exn++){
			////fprintf(stderr, " exoncount = %d\tlastexon = %d\t exn = %d\tbnode(%d,%d)\t exon(%d,%d)\n",guides[g]->exons.Count(), lastexon, exn, bundlenode->start, bundlenode->end, guides[g]->exons[exn]->start, guides[g]->exons[exn]->end);
			if(guides[g]->exons[exn]->end < bundlenode->start){ //// find overlap exon first
				lastexon = exn;
				if(exn == guides[g]->exons.Count() - 1){	//// if reach the end of guide
					if(supportState.any())
						return true;
					else
						return false;
			...
			if(bundlenode->end < guides[g]->exons[exn]->start){
				bundlenode = bundlenode->nextnode;
				break;
			}
			int start = max(guides[g]->exons[exn]->start, bundlenode->start);
			int end = min(guides[g]->exons[exn]->end, bundlenode->end);
			if(end - start <= 0)
			////if(guides[g]->exons[exn]->start < bundlenode->start || guides[g]->exons[exn]->end > bundlenode->end)
				return false;	//// if guide's exon exceed the nodes, the guide is not supported by the bundle
			else{
				supportState[exn] = 1;
				if(exn == guides[g]->exons.Count() - 1){	//// if reach the end of guide
					if(supportState.any())
...
void Merge_Bundle(int currBundleNo, int rightBundleNo, GPVec<CBundle>& bundle, GPVec<CBundlenode>& bnode)////, GList<CJunction>& junction)
{//// merge two bundles together
	int tmpNo;
	if(currBundleNo > rightBundleNo){
		tmpNo = currBundleNo;
		currBundleNo = rightBundleNo;
		rightBundleNo = tmpNo;
	}

	CBundlenode* rightBundleNode = bnode[bundle[rightBundleNo]->startnode];
	CBundlenode* currBundleNode = bnode[bundle[currBundleNo]->startnode];
	CBundlenode* bundleNode = NULL;		//// bundlenode in order here
	int bnodeid = -1;
	bool change = false;
	////fprintf(stderr, "Merging bundles\n");
	while(rightBundleNode != NULL || currBundleNode != NULL){
		/*
		if(rightBundleNode)
			fprintf(stderr, "---right id = %d (%d - %d)\n", rightBundleNode->bid, rightBundleNode->start, rightBundleNode->end);
		if(currBundleNode)
			fprintf(stderr, "---currt id = %d (%d - %d)\n", currBundleNode->bid, currBundleNode->start, currBundleNode->end);
		*/
		if(rightBundleNode == NULL && currBundleNode != NULL){
			////fprintf(stderr, "\t\tadd currBundleNode %d\n", currBundleNode->bid);
			while(currBundleNode != NULL){
				if(bnodeid != -1 && change^true){
					////add_junction(bundleNode->end, currBundleNode->start, junction, strand);
					////add_junction(bundleNode->end, currBundleNode->start, ejunction, strand);
					bundleNode->nextnode = currBundleNode;
				}
				change = true;
				bundleNode = currBundleNode;
				bnodeid = bundleNode->bid;
				currBundleNode = currBundleNode->nextnode;
				////if(currBundleNode != NULL)
				////	fprintf(stderr, "\t\tnext curr node %d\n", currBundleNode->bid);
			}
		}
		else if(rightBundleNode != NULL && currBundleNode == NULL){
			////fprintf(stderr, "\t\tadd rightBundleNode %d\n", rightBundleNode->bid);
			while(rightBundleNode != NULL){
				if(bnodeid != -1 && change^false){
					////add_junction(bundleNode->end, rightBundleNode->start, junction, strand);
					////add_junction(bundleNode->end, rightBundleNode->start, ejunction, strand);
					bundleNode->nextnode = rightBundleNode;
				}
				change = false;
				bundleNode = rightBundleNode;
				bnodeid = bundleNode->bid;
				rightBundleNode = rightBundleNode->nextnode;
				////if(rightBundleNode != NULL)
				////	fprintf(stderr, "\t\tnext right node %d\n", rightBundleNode->bid);
			}
		}
		else if(currBundleNode->bid < rightBundleNode->bid){ //// bundle nodes are in order
			////fprintf(stderr, "\t\tadd currBundleNode %d\n", currBundleNode->bid);
			if(bnodeid != -1 && change ^ true){ //// last bundle node is not from the same bundle with current bundle node
				////add_junction(bundleNode->end, currBundleNode->start, junction, strand);
				////add_junction(bundleNode->end, currBundleNode->start, ejunction, strand);
				bundleNode->nextnode = currBundleNode;
			}
		...
		else if(currBundleNode->bid > rightBundleNode->bid){
			////fprintf(stderr, "\t\tadd rightBundleNode %d\n", rightBundleNode->bid);
			if(bnodeid != -1 && change^false){
				////add_junction(bundleNode->end, rightBundleNode->start, junction, strand);
				////add_junction(bundleNode->end, rightBundleNode->start, ejunction, strand);
		...
		////fprintf(stderr, "No of junction: %d\t No of ejunction: %d \n", junction.Count(), ejunction.Count());
	}
	////if(bnode[bundle[currBundleNo]->startnode]->start > bnode[bundle[rightBundleNo]->lastnodeid]->end)
	////	fprintf(stderr, "Found bnodes not in order!\n");
	////bnode[bundle[lastBundleNo]->lastnodeid]->nextnode = bnode[bundle[currBundleNo]->startnode];
	bundle[currBundleNo]->lastnodeid = bnodeid;
	bundle[currBundleNo]->cov += bundle[rightBundleNo]->cov;
	bundle[currBundleNo]->len += bundle[rightBundleNo]->len;
	bundle[currBundleNo]->nread += bundle[rightBundleNo]->nread;
	bundle[currBundleNo]->multi += bundle[rightBundleNo]->multi;
	bundle.Delete(rightBundleNo);
	////fprintf(stderr, "finish merge\n");
}

void Correct_Node_Boundary(int refstart, int startBnode, GPVec<CBundlenode>& bnode, int g, GPVec<GffObj>& guides, GVec<float>* bpcov, char strand, GList<CJunction>& junction)
{
	int sno = 0;
	if(strand == '+')
		sno = 2;
	int lastexon = 0;
	CBundlenode* currBnode = bnode[startBnode];
	CBundlenode* lastBnode = NULL;
	for(int i = 0; i < guides[g]->exons.Count() - 1; i++)
		add_junction(guides[g]->exons[i]->end, guides[g]->exons[i + 1]->start, junction, sno-1);
	while(currBnode != NULL && lastexon != guides[g]->exons.Count()){
		////fprintf(stderr, "curr bid:%d, lastexon: %d\n", currBnode->bid, lastexon);
		for(int i = lastexon; i < guides[g]->exons.Count(); i++){
		////	fprintf(stderr, "---curr exon: %d, last exon: %d\n", i, lastexon);
			if(guides[g]->exons[i]->end < currBnode->start){	//// if current exon end before current bnode, skip to next exon
				lastexon = i + 1;
				if(lastexon == guides[g]->exons.Count() - 1)
					return;
				continue;
			}
			else if(currBnode->end < guides[g]->exons[i]->start){
				lastBnode = currBnode;
				currBnode = currBnode->nextnode;
				////lastexon = i;
				break;
				...
				lastexon = i + 1;
				/*
				if(currBnode->end > guides[g]->exons[i]->end){
					for(int nj = 0; nj < junction.Count(); nj++)	//// correct junction boundary
						if(junction[nj]->start == currBnode->end)
							junction[nj]->start = guides[g]->exons[i]->end;
					currBnode->end = guides[g]->exons[i]->end;
					if(i < guides[g]->exons.Count() - 1)
						add_junction(guides[g]->exons[i]->end, guides[g]->exons[i+1]->start, junction, strand);
				}
				*/
				if(currBnode->end < guides[g]->exons[i]->end){
					if(currBnode->nextnode != NULL)
						if(guides[g]->exons[i]->end >= currBnode->nextnode->start && verbose)
							fprintf(stderr, "Found guides's exon extend to next bnode\n");
					for(uint j = currBnode->end + 1; j <= guides[g]->exons[i]->end; j++){	//// recover bpcov for new added region
						if(((uint) j - refstart) >= ((uint)bpcov[sno].Count()))
							for(int s = 0; s < 3; s++)
								bpcov[s].Resize(j - refstart + 1, 0);
						bpcov[sno][j - refstart] += currBnode->cov/(currBnode->end - currBnode->start + 1);
					}
					currBnode->end = guides[g]->exons[i]->end;
					if(i < guides[g]->exons.Count() - 1)//// if there is still exon after current exon, add a new junction
						add_junction(guides[g]->exons[i]->end, guides[g]->exons[i+1]->start, junction, sno - 1);
				}
				/*
				if(currBnode->start < guides[g]->exons[i]->start){
					for(int nj = 0; nj < junction.Count(); nj++)
						if(junction[nj]->end == currBnode->start)
							junction[nj]->end = guides[g]->exons[i]->start;
					currBnode->start = guides[g]->exons[i]->start;
					if(i > 0)
						add_junction(guides[g]->exons[i - 1]->end, guides[g]->exons[i]->start, junction, strand);
				}
				*/
				if(currBnode->start > guides[g]->exons[i]->start){
					if(lastBnode != NULL)
						if(lastBnode->end > guides[g]->exons[i]->start && verbose)
							fprintf(stderr, "Found guide's exon extend to previous bnode\n");
					for(uint j = guides[g]->exons[i]->start; j < currBnode->start; j++)
						bpcov[sno][j - refstart] += currBnode->cov / (currBnode->end -currBnode->start + 1);
					currBnode->start = guides[g]->exons[i]->start;
					if(i > 0)
						add_junction(guides[g]->exons[i - 1]->end, guides[g]->exons[i]->start, junction, sno - 1);
				}
			}
		}
	}
...
void Correct_Bundle_Link(int refstart, int sno, GPVec<GffObj>& guides, GPVec<CBundle>& bundle, GPVec<CBundlenode>& bnode, GVec<float>* bpcov, GList<CJunction>& junction, GHash<bool>* boundary_strand)////, GList<CJunction>& ejunction)
{//// On each strand, if an identical guide support two bundles, link them together
	//// if a guide is supported by bundles but missed the links between bundles, recover the connectivities
	//// 1. group the exons of guides, overlap exons shall be in a group
	//// 2. extend the boundary of bundles if applicable(boundary is not defined by junction)
	//// 3. explore the supporting information for each guide
	//// 4. recover connectivities
	/*
	GVec<GSeg>	exon_group;
	//// 1. form group of exons
	for(int g = 0; g < guides.Count(); g++){
		int g_sno = 0;
		if(guides[g]->strand == '+')
			g_sno = 2;
		...
		int cur_id = 0;
		for(int e = 0; e < guides[g]->exons.Count(); e++){
			GSeg *cur_exon = guides[g]->exons[e];
			if(exon_group.Count() == 0){ //// add first group
				exon_group.Add(cur_exon);
				continue;
			}
			while(cur_id < exon_group.Count() && !exon_group[cur_id].overlap(cur_exon) && cur_exon->start > exon_group[cur_id].end){ //// find overlap exon_group
				cur_id++;
			}
			if(cur_id == exon_group.Count()){//// add the exon as a new group to the end of exon_group
				exon_group.Add(cur_exon);
			}
			else if(exon_group[cur_id].overlap(cur_exon)){ //// find overlap exon_group
				//// adjust boundary if necessary
				exon_group[cur_id].start = min(exon_group[cur_id].start, cur_exon->start);
				exon_group[cur_id].end = max(exon_group[cur_id].end, cur_exon->end);
				int next_id = cur_id + 1;
				while(next_id < exon_group.Count() && exon_group[cur_id].overlap(exon_group[next_id])){
					//// merge group if necessary
					exon_group[cur_id].start = min(exon_group[cur_id].start, exon_group[next_id].start);
					exon_group[cur_id].end = max(exon_group[cur_id].end, exon_group[next_id].end);
					next_id++;
				}
			}
			else{ //// insert the exon as group before the current group
				exon_group.Insert(cur_id, cur_exon);
			}
		}
	}
	//// 2. redefine the bundles
	for(int bno = 0; bno < bundle.Count(); bno++){
		CBundlenode* curr_bnode = bnode[bundle[bno]->startnode];
		CBundlenode* prev_bnode = NULL;
		while(curr_bnode != NULL){
			GStr cur_bnode_start((int)curr_bnode->start);
			GStr cur_bnode_end((int)curr_bnode->end);
			if(!boundary_strand[cur_bnode_start]){ //// 5' end extenable
				for(int eg = 0; eg < exon_group.Count(); eg++){
					if(exon_group[eg].overlap(curr_bnode->start, curr_bnode->end) && exon_group[eg]->start < curr_bnode->start){ //// update 5' end
						curr_bnode->start = exon_group[eg]->start;
						break;
					}
				}
			}
			if(!boundary_strand[cur_bnode_end]){//// 3' end extenable
				for(int eg = 0; eg < exon_group.Count(); eg++){
					if(exon_group[eg].overlap(curr_bnode->start, curr_bnode->end) && exon_group[eg]->end > curr_bnode->end){
						curr_bnode->end = exon_group[eg]->end;
						break;
					}
				}
			}
			prev_bundle = curr_bnode;
			curr_bnode = curr_bnode->nextnode;
		}
	}*/
	////GVec<int> bridgeGuides;		//// records the guides which span more than one bundle
	////GVec<int>* gSpanedBundle = new GVec<int>[guides.Count()];	//// records the indexes of bundles spanned by guides
	////GBitVec gSupportState[guides.Count()][bundle.Count()];	//// records the state of exon whether is it supported by bundle node
	////fprintf(stderr, "Looking for support guide, starting from %d\n", bnode[bundle[0]->startnode]->start);
	////fprintf(stderr, "first node(%d-%d)\n", bnode[0]->start, bnode[0]->end);

	for(int g = 0; g < guides.Count(); g++){
		//fprintf(stderr, "%d-th guide\n", g);
		int gsno = 0;
		bool added = false;
		if(guides[g]->strand == '-')
			gsno = 0;
		else if(guides[g]->strand == '+')
			gsno = 2;
		if(gsno != sno) //// if not the same strand, skip the guide
			continue;
		GVec<int> gSpanedBundle;								//// records the indexes of bundles spanned by guide
		GBitVec gSupportState[bundle.Count()];					//// records the state of exons whether are they supported by bundle node
		////GBitVec supportState(guides[g]->exons.Count());
		for(int bno = 0; bno < bundle.Count(); bno++){ 			//// find the overlap bundles
			////if(bundle[bno]->cov / bundle[bno]->len > readthr)	//// if the mean depth is over threshold(2.5 here), we skip the bundle.(It is not possible to lose link)
			////	continue;
			uint bundleStart = bnode[bundle[bno]->startnode]->start;  //// records the start of current bundle
			uint bundleEnd = bnode[bundle[bno]->lastnodeid]->end;		//// records the end of current bundle
			//fprintf(stderr, "    %d-th bundle(%d, %d)\n", bno, bundleStart, bundleEnd);
			if(bundleEnd <= guides[g]->start)  					//// if bundle ends before the guide, skip the bundle
				continue;
			if(bundleStart >= guides[g]->end)		//// if guide end before bundle, go to next guide
				break;
			if(guides[g]->start < bundleStart || guides[g]->end > bundleEnd){ //// if guide exceeds the bundle, find a protential bridge guide
				GBitVec supportState(guides[g]->exons.Count());
				if(Is_Bundle_Support_Guide(g, bundle[bno]->startnode, bnode, guides, supportState, boundary_strand)){
					added = true;
					//fprintf(stderr,"   state:");
					//printBitVec(supportState);
					gSupportState[bno] = supportState;
					gSpanedBundle.cAdd(bno);
				}
			}
		}
		if(added && gSpanedBundle.Count() > 1){
			GBitVec support(guides[g]->exons.Count());
			for(int bno = 0; bno < bundle.Count(); bno++)
				support |= gSupportState[bno];
			if(support.all()){
				int rightBundleNo = gSpanedBundle.Pop();
				((RC_TData*)(guides[g]->uptr))->in_bundle = 2;
				//fprintf(stderr, "-%d spanned bundle in total\n", gSpanedBundle.Count());
				for(int bID = gSpanedBundle.Count() - 1; bID >=0; bID--){
					int currBundleNo = gSpanedBundle[bID];
					Merge_Bundle(currBundleNo, rightBundleNo, bundle, bnode);	////, junction);
					rightBundleNo = currBundleNo;
				}
				int startBnode = bundle[gSpanedBundle[0]]->startnode;
				Correct_Node_Boundary(refstart, startBnode, bnode, g, guides, bpcov, guides[g]->strand, junction);
			}
		}
	}
	////fprintf(stderr, "Finish looking\n");
}
...
*/
	...
	//// records the boundaries on each strand
	GHash<bool> boundary_neg; //// boundaries on negative strand
	GHash<bool>	boundary_pos; //// boundaries on positive strand
		...
		*/
		...
		while(i<rd.juncs.Count()) {
			...
			if(!jd.strand || !good_junc(jd,refstart,bpcov)) { // found a bad junction
				...
				//// records the information of boundary
				if(jd.strand == '+'){
					if(!boundary_pos[bs.chars()])
						boundary_pos.Add(bs.chars(), new bool(exist));
					if(!boundary_pos[be.chars()])
						boundary_pos.Add(be.chars(), new bool(exist));
				}else{
					if(!boundary_neg[bs.chars()])
						boundary_neg.Add(bs.chars(), new bool(exist));
					if(!boundary_neg[bs.chars()])
						boundary_neg.Add(be.chars(), new bool(exist));
				}
			}
			i++;
		}
...
/*
for(int sno = 0; sno < 3; sno += 2){
	fprintf(stderr, "strand: %d\n", sno);
	if(bnode[sno].Count())
		for(int i = 0; i < bnode[sno].Count(); i++){
			fprintf(stderr, "%d-node:(%d-%d)\t", i, bnode[sno][i]->start, bnode[sno][i]->end);
			if(i%5==4)
				fprintf(stderr,"\n");
		}
}
for(int j = 0; j < junction.Count(); j++){
	fprintf(stderr, "%d-junc:(%d-%d):%d\t", j, junction[j]->start, junction[j]->end, junction[j]->strand);
	if(j%5==4)
		fprintf(stderr, "\n");
}
...
*/
if(guides.Count()){
	//eliminate_transfrags_under_thr
	for(int sno = 0; sno < 3; sno += 2)	//// correct link on each strand
		if(bundle[sno].Count()){
			GHash<bool> *boundary_strand;
			if(sno == 0)
				boundary_strand = &boundary_neg;
			else
				boundary_strand = &boundary_pos;
			Correct_Bundle_Link(refstart, sno, guides, bundle[sno], bnode[sno], bpcov, junction, boundary_strand);////, ejunction);
		}
	boundary_neg.Clear();
	boundary_pos.Clear();
}
...
fprintf(stderr,"\n");
for(int g = 0; g < guides.Count(); g++){
	fprintf(stderr, "%d guide: ", g);
	for(int i = 0; i < guides[g]->exons.Count(); i++){
		fprintf(stderr, "%d-exon:(%d-%d)\t", i, guides[g]->exons[i]->start, guides[g]->exons[i]->end);
		if(i%5==4)
			fprintf(stderr,"\n");
	}
}
			...
			for(int b=0;b<bundle[sno].Count();b++) {
				...
				if(bundle[sno][b]->nread &&
						(((bundle[sno][b]->multi/bundle[sno][b]->nread)<=mcov && bundle[sno][b]->len >= mintranscriptlen)
						 ||nolap || guides.Count() > 0)) { // bundle is worth processing: it might be that there are small transfrags from source to sink that are worth processing
	...
	for(int s=0;s<2;s++) {
		...
		for(int b=0;b<bno[s];b++) {
			if(graphno[s][b]) {
				...
				if(guides.Count()) {
					process_refguides(graphno[s][b],edgeno[s][b],gpos[s][b],lastgpos[s][b],no2gnode[s][b],transfrag[s][b],s,guides,guidetrf,bdata);
					if(guidetrf.Count() == 0)
						continue;
				}
				////newly added
				edgeno[s][b] = lastgpos[s][b] - graphno[s][b]; 
				GBitVec maskbit(lastgpos[s][b]);
				for(int i = 0; i < transfrag[s][b].Count(); i++)
					transfrag[s][b][i]->pattern |= maskbit;
				for(int i = 0; i < guidetrf.Count(); i++)
					guidetrf[i].trf->pattern |= maskbit;
...
*/
				...
				bool state = false;
				fprintf(stderr, "%s(%d-%d) s = %d, b = %d\n", bdata->refseq.chars(), bdata->start, bdata->end, s, b);
				/*
				   for(int tmpi = 0; tmpi < guidetrf.Count(); tmpi++){
				   if(!strcmp(guides[guidetrf[tmpi].g]->getGeneID(), "NR_031754") || 
				   !strcmp(guides[guidetrf[tmpi].g]->getGeneID(), "NR_030741"))
				   state = true;

				   fprintf(stderr, "guide %s\t: ", guides[guidetrf[tmpi].g]->getGeneID());
				   for(int tj = 0; tj < guidetrf[tmpi].trf->nodes.Count(); tj++){
				   fprintf(stderr, "%d\t", guidetrf[tmpi].trf->nodes[tj]);
				   }
				   fprintf(stderr,"\n");
				   }
				   fprintf(stderr, "\n");
				...
				if(!state){
					geneno=find_transcripts(graphno[s][b],edgeno[s][b],gpos[s][b],no2gnode[s][b],transfrag[s][b],compatible, geneno,s,guidetrf,pred,guides,guidepred,bdata, tr2no[s][b]);
					/*
					   if(pred.Count()>0)
					   {
					   for(int tmpi = 0; tmpi < pred.Count(); tmpi++)
					   {
					   fprintf(stderr, "%d (%c) : ", tmpi, pred[tmpi]->strand);
					   if (pred[tmpi]->t_eq)
					   fprintf(stderr,"%s\t", pred[tmpi]->t_eq->getID());
					   for(int tj = 0; tj < pred[tmpi]->exons.Count(); tj++){
					   fprintf(stderr, "%d-%d ", pred[tmpi]->exons[tj].start, pred[tmpi]->exons[tj].end);
					   }
					   fprintf(stderr, "\n");
					   }
					   fprintf(stderr, "\n");
	  ...
	  if(pred[n]->t_eq || (!eonly && is_pred_above_frac(maxpos,pred[n]))) { // print this transcript only if it's matching known gene or if not eonly and above threshold
		  ...
		  */
		  ...
		  if(pred[n]->flag) {
			  ...
			  fprintf(f_out,"%s\tIsoRef\ttranscript\t%d\t%d\t1000\t%c\t.\tgene_id \"%s\"; transcript_id \"%s\"; ",
						  refname.chars(),pred[n]->start,pred[n]->end,pred[n]->strand,geneid.chars(),trid.chars());
			  ...
			  fprintf(f_out,"cov \"%.6f\";\n",pred[n]->cov);
			  for(int j=0;j<pred[n]->exons.Count();j++) {
				  fprintf(f_out,"%s\tIsoRef\texon\t%d\t%d\t1000\t%c\t.\tgene_id \"%s\"; transcript_id \"%s\"; exon_number \"%d\"; ",
						  refname.chars(),pred[n]->exons[j].start,pred[n]->exons[j].end,pred[n]->strand,geneid.chars(),
						  trid.chars(),j+1); // maybe add exon coverage here
				  if(pred[n]->t_eq) {
					  if(!eonly) fprintf(f_out,"reference_id \"%s\"; ",pred[n]->t_eq->getID());
	...
	for(int i=0;i<keep.Count();i++) {
	  ...
	  if(pred[n]->t_eq || (!eonly && is_pred_above_frac(maxpos,pred[n]))) { // print this transcript only if it's matching known gene or if not eonly and above threshold
		  ...
		  if(pred[n]->flag) {
			  ...
			  //fprintf(f_out,"%d %d %d %.6f %.6f\n",pred[n]->exons.Count()+1,pred[n]->tlen, t_id, pred[n]->frag,pred[n]->cov);
			  fprintf(f_out,"%d %d %d %.6f\n",pred[n]->exons.Count()+1,pred[n]->tlen, t_id, pred[n]->cov);
			  fprintf(f_out,"%s\tIsoRef\ttranscript\t%d\t%d\t1000\t%c\t.\tgene_id \"%s.%d\"; transcript_id \"%s.%d.%d\"; ",
					  refname.chars(),pred[n]->start,pred[n]->end,pred[n]->strand,label.chars(),genes[pred[n]->geneno],
					  label.chars(),genes[pred[n]->geneno],transcripts[pred[n]->geneno]);
			  if(pred[n]->t_eq) {
				  fprintf(f_out,"reference_id \"%s\"; ",pred[n]->t_eq->getID());
			  ...
			  fprintf(f_out,"cov \"%.6f\";\n",pred[n]->cov);
			  for(int j=0;j<pred[n]->exons.Count();j++) {
				  fprintf(f_out,"%s\tIsoRef\texon\t%d\t%d\t1000\t%c\t.\tgene_id \"%s.%d\"; transcript_id \"%s.%d.%d\"; exon_number \"%d\"; ",
						  refname.chars(),pred[n]->exons[j].start,pred[n]->exons[j].end,pred[n]->strand,label.chars(),genes[pred[n]->geneno],
						  label.chars(),genes[pred[n]->geneno],transcripts[pred[n]->geneno],j+1); // maybe add exon coverage here
				  if(pred[n]->t_eq) {
					  fprintf(f_out,"reference_id \"%s\"; ",pred[n]->t_eq->getID());
		...
		if(keep[n]){
			...
			while(m<npred && pred[m]->start<=currentend) {
				if(keep[m] && pred[m]->geneno ==pred[n]->geneno) { // this is a prediction worth printing
					...
					fprintf(f_out,"%s\tIsoRef\ttranscript\t%d\t%d\t1000\t%c\t.\tgene_id \"%s.%d\"; transcript_id \"%s\"; ",
					...
					for(int j=0;j<pred[m]->exons.Count();j++) {
						fprintf(f_out,"%s\tIsoRef\texon\t%d\t%d\t1000\t%c\t.\tgene_id \"%s.%d\"; transcript_id \"%s\"; exon_number \"%d\"; ",
