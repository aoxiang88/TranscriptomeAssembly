#ifndef __RLINK_H__
#define __RLINK_H__
#include <cmath> //// for float abs(float)
#include "GArgs.h"
#include "GStr.h"
#include "gff.h"
#include "GBam.h"
...
#include "GIntHash.hh"
#include <ilcplex/ilocplex.h> //// to use CPLEX library
ILOSTLBEGIN
...
/*
//// struct for storing the infromation of overlap regions among guides' exons
struct CExonGroup {
	int start;
	int end;
	GVec<int> g;
	GVec<int> exon;
	CExonGroup *next_eg;
	CExonGroup(int gstart=0, int gend=0, CExonGroup *_next_gr=NULL):start(gstart),end(gend),g(),exon(), next_eg(_next_eg) {}
};
*/
