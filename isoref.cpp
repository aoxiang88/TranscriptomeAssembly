...
#define USAGE "IsoRef v" VERSION " usage:\n\
 isoref <input.bam > <-g <guide_gff> [-l <label>] \n\
  [-o <out_gtf>] [-p <cpus>] [-v] [-a <min_anchor_len>] \n\
  [-m <min_tlen>] [-j <min_anchor_cov>] [-f <min_iso>]\n\
...
bool balance = true; //// switch for balancing flow of splice graph
...
bool forceBAM = false; //useful for stdin (piping alignments into IsoRef)
...
int main(int argc, char * const argv[]) {
 ...
 GArgs args(argc, argv, 
   ...
   "debug;help;version;keeptmp;bam;fr;rf;merge;disbal;exclude=zZSEIhvteux:n:s:D:G:C:l:m:o:a:j:c:f:p:g:P:M:Bb:A:F:T:");
...
if(!mergeMode) {
	...
	fprintf(f_out,"# IsoRef version %s\n",VERSION);
...
void processOptions(GArgs& args) {
	...
	if (args.getOpt("rf")) {
	 ...
	 if (verbose) {
	     fprintf(stderr, "Running IsoRef " VERSION ". Command line:\n");
	...
	*/
	if(args.getOpt("disbal"))
	 ...
	 else if(mergeMode) label="MIsoRef";
	...
	*/
	...
	if(out_dir[strlen(out_dir.chars()) - 1] != '/')
		out_dir += '/';
	*/
